...
import PicViewerMobile from '@/components/PicViewerMobile'

export interface PicViewerDialogProps {
  open: boolean
...
export interface PicViewerDesktopProps {
...
function getRotatedBoundingSize(width: number, height: number, degrees: number): { width: number; height: number } {
...
function resolveDownloadFileName(displayPath: string, alt: string | undefined, mimeType: string): string {
  const rawName = (displayPath || alt || 'image').split(/[\\/]/).filter(Boolean).at(-1) || 'image'
  // 去除 URL 查询串与锚点后再取文件名（新闻图片地址常带 ?w=…&h=…）。
  const noQuery = rawName.split(/[?#]/)[0]
  const safeName = noQuery.replace(/[<>:"/\\|?*\x00-\x1F]/g, '_').trim() || 'image'
 ...
 * 通用图片查看器（桌面端预览弹窗）。
 *
 * 支持：缩放 / 旋转 / 镜像 / 反色 / 底色 / 拖拽 / 原图尺寸信息 / 下载，
 * 可选上一张 / 下一张（供多图阅读页使用）。
 */
function PicViewerDialog({
  open,
  onOpenChange,
  src,
  alt,
  ...
  const [imageMousePosition, setImageMousePosition] = useState<{ x: number; y: number } | null>(null)
  ...
  viewRef.current = { scale, x: translateX, y: translateY }
  const displayBlobSize = blobSize
  ...
  const bgModes: Array<'dark' | 'light' | 'grid'> = ['dark', 'light', 'grid']
  const bgModeLabels: Record<string, string> = { dark: '黑色', light: '白色', grid: '网格' }
  ...
  }, [handleFitToScreen, open, scheduleFitToView, src])
  ...
  }, [hasNext, hasPrev, onNext, onPrev, open])

  // 非被动滚轮监听，避免阻止默认滚动失效；以鼠标位置为中心缩放。
  // 依赖 containerNode 保证 DOM 元素已挂载后再绑定事件。
  useEffect(() => {
    if (!open || !containerNode) return undefined
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      const newScale = clampScale(oldScale - delta * 0.003)
      if (newScale === oldScale) return

      // 鼠标相对容器中心的偏移，以及相对图片中心（容器中心 + 平移量）的锚点。
      const offsetX = mouseX - centerX
      const offsetY = mouseY - centerY
      const anchorX = offsetX - viewRef.current.x
      const anchorY = offsetY - viewRef.current.y
      const ratio = newScale / oldScale

      // 保持鼠标下的图片点不动：新平移量 = 鼠标偏移 - 缩放后的锚点偏移。
      setScale(newScale)
      setTranslateX(offsetX - anchorX * ratio)
      setTranslateY(offsetY - anchorY * ratio)
      ...
      wheelTimerRef.current = window.setTimeout(() => setWheeling(false), 120)
    }
              ...
              >
                ...
                {hasPrev && (
                  <button
                    ...
                    className="absolute left-3 top-1/2 z-10 flex size-10 -translate-y-1/2 items-center justify-center rounded-full border border-white/20 bg-black/40 text-white/90 transition hover:bg-black/60"
                    aria-label="上一张"
                  >
                ...
                {hasNext && (
                  <button
                    ...
                    onMouseDown={(e) => e.preventDefault()}
                    onPointerDown={(e) => e.stopPropagation()}
                    className="absolute right-3 top-1/2 z-10 flex size-10 -translate-y-1/2 items-center justify-center rounded-full border border-white/20 bg-black/40 text-white/90 transition hover:bg-black/60"
                    aria-label="下一张"
                  >
                ...
                <img
                  ...
                  style={{
                    ...
                    transition: dragging || wheeling ? 'none' : 'transform 0.2s ease',
...
/**
 * 桌面端缩略图查看器：显示小图，点击打开预览弹窗。
 * 支持两种取图方式：
 * - src 模式：直接使用传入地址（远程 URL 等）。
 * - fetcher 模式：通过异步函数拉取 Blob 并创建对象 URL（用于鉴权接口）。
 */
function PicViewerDesktop({
  fetcher,
  src,
  alt,
  width = 80,
...
/**
 * 自适应图片查看器：按设备类型选择加载移动端或桌面端查看器。
 * 移动端（手机 / 平板）走 PicViewerMobile；其余走桌面端 PicViewerDesktop。
 */
function PicViewer(props: PicViewerDesktopProps) {
  const isMobile = useMemo(() => isMobileDevice(), [])
  return isMobile ? <PicViewerMobile {...props} /> : <PicViewerDesktop {...props} />
}

export { PicViewerDialog, PicViewerDesktop }
